...
class CameraTrajectory:
    ...
    def add_camera(self, keyframe: Keyframe, keyframe_index: int | None = None) -> None:
        ...
        print(f"{keyframe.wxyz=} {keyframe.position=} {keyframe_index=} {keyframe.aspect=}")
        ...
        def _(_) -> None:
            ...
            with server.scene.add_3d_gui_container("/camera_edit_panel", 
                                                    position=keyframe.position) as camera_edit_panel:
                self._camera_edit_panel = camera_edit_panel

                override_fov = server.gui.add_checkbox(
                    "Override FOV", 
                    initial_value=keyframe.override_fov_enabled
                )
            ...
            def _(event: viser.GuiEvent) -> None:
                assert event.client is not None
                client = event.client
                T_world_current = vt.SE3.from_rotation_and_translation(vt.SO3(client.camera.wxyz), client.camera.position)
                T_world_target  = vt.SE3.from_rotation_and_translation(vt.SO3(     keyframe.wxyz),      keyframe.position) @ \
                                  vt.SE3.from_translation(np.array([0.0, 0.0, -0.5]))

                T_current_target = T_world_current.inverse() @ T_world_target

                for j in range(10):
                    T_world_set = T_world_current @ vt.SE3.exp(T_current_target.log() * j / 9.0)

                    # NOTE: we atomically set both orientation and position of camera.
                    with client.atomic():
    ...
    def spline_t_from_t_sec(self, time: np.ndarray) -> np.ndarray:
        """
        From a time value in seconds, compute a t value for our geometric spline interpolation. 
        An increment of 1 for the latter will move the camera forward by 1 keyframe.
        ...
        if self.loop:
            ...
            interpolator = scipy.interpolate.PchipInterpolator(x=transition_times_cumsum, 
                                                               y=spline_indices)
    ...
    def interpolate_pose_and_fov_rad(
        ...
        self._fov_spline = splines.KochanekBartels(
            [
                 ...
                 if keyframe[0].override_fov_enabled else self.default_fov
                for keyframe in self._keyframes.values()
            ],
    ...
    def update_spline(self) -> None:
        ...
        def make_transition_handle(i: int) -> None:
            ...
            def _(_) -> None:
                ...
                with server.scene.add_3d_gui_container("/camera_edit_panel", 
                                                        position=transition_pos) as camera_edit_panel:
                    self._camera_edit_panel = camera_edit_panel
...
def load_gui(
    ...
    **kwargs,
    ...
    with server.gui.add_folder("Preset camera trajectories", order=99, expand_by_default=False):
        preset_traj_dropdown = server.gui.add_dropdown(
            "Options",
            ["orbit","spiral","lemniscate","zoom-out","dolly zoom-out"],
            initial_value="orbit",
        ...
        def _(event: viser.GuiEvent) -> None:
            ...
            start_w2c = torch.linalg.inv(
                torch.as_tensor(
                    vt.SE3.from_rotation_and_translation(vt.SO3(camera.wxyz), camera.position).as_matrix(),
                    dtype=torch.float32,
                )
            )
            ...
            for pose, fov in zip(poses, fovs):
                camera_traj.add_camera(
                    Keyframe.from_se3(vt.SE3.from_matrix(pose),
                                      fov=fov,
    ...
    def add_preview_frame_slider() -> viser.GuiInputHandle[int] | None:
        """
        Helper for creating the current frame # slider. 
        This is removed and re-added anytime the `max` value changes.
        """
