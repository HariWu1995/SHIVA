...
class Decoder(nn.Module):
    ...
    def forward(self, features, gt_norm_mask=None, mode='test'):
        ...
        if mode == 'train':
            ...
            # samples: [B, 1, N, 2]
            point_coords_res4, \
            rows_int, cols_int = sample_points(out_res8_res4.detach(), gt_norm_mask,
        ...
        if mode == 'train':
            ...
            # samples: [B, 1, N, 2]
            point_coords_res2, \
            rows_int, cols_int = sample_points(out_res4_res2.detach(), gt_norm_mask,
                                                sampling_ratio=self.sampling_ratio,
                                                        beta=self.importance_ratio)
        ...
        if mode == 'train':
            ...
            # samples: [B, 1, N, 2]
            point_coords_res1, \
            rows_int, cols_int = sample_points(out_res2_res1.detach(), gt_norm_mask,
                                                sampling_ratio=self.sampling_ratio,
                                                        beta=self.importance_ratio)
