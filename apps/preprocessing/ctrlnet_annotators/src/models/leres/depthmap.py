...
def calculate_processing_res(img, base_size, confidence=0.1, scale_threshold=3, whole_size_threshold=3000):
    """
    Returns the R_x resolution described in section 5 of the main paper.

    Parameters:
       img :input rgb image
       base_size : size the dilation kernel which is equal to receptive field of the network.
    ...
    kernel = np.ones((int(base_size/speed_scale), int(base_size/speed_scale)), float)
    
    # dilation kernel with size of the a quarter of receptive field used to compute k as described in section 6 of main paper
    kernel2 = np.ones((int(base_size / (4*speed_scale)), int(base_size / (4*speed_scale))), float)
    ...
    for p_size in range(int(base_size / speed_scale), 
        ...
        mean_value = (1 - dilated).mean()
        if mean_value > confidence:
            break
        else:
...
def single_estimate(img, msize, model, net_type):
    ...
    #     return estimate_leres(img, model, msize, msize)
    # else:
    #     # FIXME: https://github.com/graemeniedermayer/stable-diffusion-webui-normalmap-script/blob/main/scripts/normalmap.py#L461
...
def apply_grid_patch(blsize, stride, img, box):
    ...
    for k in range(blsize, img.shape[1] - blsize, stride):
        for j in range(blsize, img.shape[0] - blsize, stride):
            ...
            patchbounds = [j - blsize, 
                           k - blsize, 
                           j - blsize + 2 * blsize, 
                           k - blsize + 2 * blsize]
            patch_bound = [box[0] + patchbounds[1], 
                           box[1] + patchbounds[0], 
...
def generate_patches(img, base_size):
    ...
    # Refine initial Grid of patches by discarding the flat 
    #       (in terms of gradients of the rgb image) ones.
    # Refine each patch size to ensure that there will be enough depth cues 
    #       for the network to generate a consistent depth map.
    print("Selecting patches ...")
    patch_bound_list = adaptive_selection(grad_integral_image, patch_bound_list, gf)

    # Sort the patch list to make sure the merging operation will be done with the correct order:
    # starting from biggest patch
...
class ImageAndPatches:
    ...
    def __init__(self, root_dir, name, patchs_info, rgb_image, scale=1):
        self.root_dir = root_dir
        self.patchs_info = patchs_info
        self.name = name
        self.patchs = patchs_info
        self.scale = scale

        scale_w = round(rgb_image.shape[1]*scale)
        scale_h = round(rgb_image.shape[0]*scale)
        self.rgb_image = cv2.resize(rgb_image, (scale_w, scale_h), interpolation=cv2.INTER_CUBIC)
    ...
    def __getitem__(self, index):
        ...
        patch_rgb = im_patch(self.rgb_image, rect)
        ...
        if self.do_have_estimate:
            output.update({
                'patch_whole_estimate_base': im_patch(self.estimation_base_image, rect),
                'patch_whole_estimate_updated': im_patch(self.estimation_updated_image, rect), 
            })
...
def estimate_boost(img, model, model_type, pix2pix_model, max_res=512, **kwargs):
    ...
    # The resolution search described in section 5-double estimation of the main paper 
    #                                and section B of the supplementary material.
    whole_image_optimal_size, \
        patch_scale = calculate_processing_res(img, net_receptive_field_size, 
                                            r_threshold_value, scale_threshold, whole_size_threshold)
    ...
    if max_res < whole_image_optimal_size:
        # print("No Local boosting. Specified Max Res is smaller than R20, Returning double_estimate result")
        return cv2.resize(whole_estimate, (input_resolution[1], 
                                           input_resolution[0]), interpolation=cv2.INTER_CUBIC)
    ...
    # NOTE that our method output resolution is independent of the input resolution 
    # and this parameter will only enable a scaling operation during the local patch merge implementation 
    # to generate results with the same resolution as the input.
    ...
    mergein_scale = input_resolution[0] / img.shape[0]
    mergein_w = round(img.shape[1] * mergein_scale)
    mergein_h = round(img.shape[0] * mergein_scale)

    image_and_patches = ImageAndPatches('', '', patch_set, img, mergein_scale)

    whole_estimate_resized = cv2.resize(whole_estimate, (mergein_w, mergein_h), interpolation=cv2.INTER_CUBIC)
    ...
    for patch_ind in range(len(image_and_patches)):
        ...
        print('\t Processing patch', patch_ind, '/', len(image_and_patches)-1, '|', rect)
        ...
        pix2pix_model.set_input(patch_whole_estimate_base, patch_estimation)
        ...
        # We use a simple linear polynomial to make sure the result of the merge network 
        #   would match the values of base estimate
        ...
        # To speed up the implementation, we only generate the Gaussian mask once 
        # with a sufficiently large size and resize it to our needed size while merging the patches.
        if mask.shape != org_size:
            mask = cv2.resize(mask_org, (org_size[1], org_size[0]), interpolation=cv2.INTER_LINEAR)

        to_be_merged = image_and_patches.estimation_updated_image

        # Update the whole estimation:
        # We use a simple Gaussian mask to blend the merged patch region with the base estimate 
        # to ensure seamless blending at the boundaries of the patch region.
        to_be_merged[h1:h2, w1:w2] = np.multiply(to_be_merged[h1:h2, w1:w2], 1 - mask) + np.multiply(merged, mask)
        image_and_patches.set_updated_estimate(to_be_merged)

    # output
    return cv2.resize(image_and_patches.estimation_updated_image, 
                    (input_resolution[1], input_resolution[0]), interpolation=cv2.INTER_CUBIC)
