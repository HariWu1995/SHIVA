...
class Pix2Pix4DepthModel(BaseModel):
    ...
    By default, it uses 
        a '--netG unet256' U-Net generator,
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=2e-6, betas=(opt.beta1, 0.999))
    ...
    def set_input_train(self, input):
        ...
        self.outer = F.interpolate(self.outer, (1024, 1024), mode='bilinear', align_corners=False)
        self.inner = F.interpolate(self.inner, (1024, 1024), mode='bilinear', align_corners=False)

        self.image_paths = input['image_path']

        if self.isTrain:
            self.gtfake = input['data_gtfake'].to(self.device)
            self.gtfake = F.interpolate(self.gtfake, (1024, 1024), mode='bilinear', align_corners=False)
