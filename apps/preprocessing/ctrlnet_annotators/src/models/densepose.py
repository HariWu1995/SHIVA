...
class BoxMode(IntEnum):
    ...
    """
    XYXY_ABS = 0    # (x0, y0, x1, y1) in absolute FP coordinates & in range [0, width / height]
    XYWH_ABS = 1    # (x0, y0, w, h) in absolute FP coordinates

    XYWHA_ABS = 4   # (xc, yc, w, h, a) in absolute FP coordinates
                    # (xc, yc) is the center of the rotated box, 
                    # a is the angle in degrees ccw (counter-clock-wise)

    # NOTE: Not yet supported!
    XYXY_REL = 2    # (x0, y0, x1, y1) in range [0, 1] & relative to image size
    XYWH_REL = 3    # (x0, y0, w, h) in range [0, 1] & relative to image size
    ...
    def convert(box: _RawBoxType, from_mode: "BoxMode", to_mode: "BoxMode") -> _RawBoxType:
        ...
        if from_mode == BoxMode.XYWHA_ABS \
         and to_mode == BoxMode.XYXY_ABS:
        ...
        elif from_mode == BoxMode.XYWH_ABS \
           and to_mode == BoxMode.XYWHA_ABS:
            original_dtype = arr.dtype
            arr = arr.double()
            arr[:, 0] += arr[:, 2] / 2.0
            arr[:, 1] += arr[:, 3] / 2.0
...
class MatrixVisualizer:
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh):
        ...
        if np.any(matrix_scaled > 255):
            print("[Warning] Matrix has values > 255 after scaling, clipping to [0..255]")
...
class DensePoseMaskedColormapResultsVisualizer(DensePoseResultsVisualizer):
    ...
    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh) -> None:
        ...
        segment = self.segm_extractor(iuv_arr)
...
def densepose_chart_predictor_output_to_result_with_confidences(
    ...
    labels = resample_refine_and_coarse_segm_tensors_to_bbox(refine_segm, coarse_segm, box_xywh).squeeze(0)
    uv = resample_uv_tensors_to_bbox(u, v, labels, box_xywh)
    confidences = []
    return box_xywh, labels, uv


def resample_refine_and_coarse_segm_tensors_to_bbox(
    refine_segm: torch.Tensor, 
    coarse_segm: torch.Tensor, 
    box_xywh_abs: IntTupleBox,
):
    """
    Resample refine and coarse segmentation tensors to the given
    bounding box and derive labels for each pixel of the bounding box

    Args:
        refine_segm: float tensor of shape [1, C, Hout, Wout]
    ...
    refine_segm_bbox = F.interpolate(refine_segm, (h, w), mode="bilinear", align_corners=False).argmax(dim=1) \

    labels = refine_segm_bbox * (coarse_segm_bbox > 0).long()
    return labels
...
def resample_uv_tensors_to_bbox(
    ...
    Args:
        ...
        labels (tensor [H, W] of long): labels obtained by resampling segmentation outputs 
