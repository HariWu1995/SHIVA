...
from ..conv_encoder_decoder.submodules import (
    Encoder, UpSampleBN, UpSampleGN, 
    INPUT_CHANNELS_DICT, upsample_via_bilinear, upsample_via_mask, 
    get_prediction_head,
)
...
class DSINE_v00(nn.Module):
    ...
    def __init__(self, args):
        ...
        print('Defining DSINE_v00 (baseline encoder-decoder, w/o ray encoding)')
        print('B: %s / NF: %s / BN: %s'  % (B, NF, BN))
        print('output_dim: %s / down: %s / learned upsampling: %s' % (args.NNET_output_dim, down, learned_upsampling))
        self.encoder = Encoder(B=B, pretrained=True)
        self.decoder = Decoder(num_classes=args.NNET_output_dim, B=B, NF=NF, BN=BN, 
