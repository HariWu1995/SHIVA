...
class DenseDepth(nn.Module):
    ...
    def __init__(self, num_classes, B=5, pretrained=True, NF=2048, BN=True,
                            down=2, learned_upsampling=True, **kwargs):
        super(DenseDepth, self).__init__()
        self.encoder = Encoder(B=B, pretrained=pretrained)
        self.decoder = Decoder(num_classes=num_classes, B=B, NF=NF, BN=BN,
                               down=down, learned_upsampling=learned_upsampling, **kwargs)
