...
def smart_resize(x, s):
    ...
    if Co == 3 or Co == 1:
        ...
        return cv2.resize(x, (int(Wt), int(Ht)), interpolation=cv2.INTER_AREA if k < 1 else \
                                                               cv2.INTER_LANCZOS4)
    else:
...
def transfer(model, model_weights):
    ...
    for weights_name in model.state_dict().keys():
        transfered_weights[weights_name] = model_weights['.'.join(weights_name.split('.')[1:])]
    return transfered_weights


def is_normalized(keypoints: List[Optional[Keypoint]]) -> bool:
    point_normalized = [
            0 <= abs(k.x) <= 1 
        and 0 <= abs(k.y) <= 1 for k in keypoints if k is not None
    ]
...
def draw_handpose(canvas: np.ndarray, keypoints: Union[List[Keypoint], None]) -> np.ndarray:
    ...
    for ie, (e1, e2) in enumerate(edges):
        ...
        if x1 > eps and y1 > eps and x2 > eps and y2 > eps:
            color = hsv_to_rgb([ie / float(len(edges)), 1.0, 1.0]) * 255
...
def handDetect(body: BodyResult, image) -> List[Tuple[int, int, int, bool]]:
    """
    ...
    Args:
        ...
        image (numpy.ndarray): A 3D numpy array representing the original input image.
...
# Written by Lvmin
def faceDetect(body: BodyResult, image) -> Union[Tuple[int, int, int], None]:
    """
