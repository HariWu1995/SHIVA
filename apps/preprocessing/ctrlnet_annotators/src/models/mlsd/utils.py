...
def pred_lines(
    ...
    for center, score in zip(pts, pts_score):
        ...
        if score > score_thresh and \
        distance > dist_thresh:
            disp_x_start, disp_y_start, \
            disp_x_end,   disp_y_end = vmap[y, x, :]
            x_start = x + disp_x_start
            y_start = y + disp_y_start
            x_end = x + disp_x_end
            y_end = y + disp_y_end
            segment_list.append([x_start, y_start, x_end, y_end])

    lines = 2 * np.array(segment_list)  # 256 > 512
...
def pred_squares(
    ...
    segments = np.array(segment_list)
    ...
    # ### fast suppression using tensorflow
    ...
    ### fast suppression using pytorch
    ...
    '''
    # rename variables
    deg1_map = deg_inter_to_start
    deg2_map = deg_inter_to_end

    # sort deg ascending
    deg_sort = np.sort(np.concatenate([deg1_map[:, :, None], 
                                       deg2_map[:, :, None]], axis=-1), axis=-1)
            ...
            for corner2 in corner_dict[2]:
                ...
                for corner3 in corner_dict[3]:
                    ...
                    for corner3_line in corner3[2:]:
                        if corner3_line not in corner0[2:]:
                            continue
                        square_list.append(corner0[:2] + corner1[:2] + corner2[:2] + corner3[:2])
                        segment_list.append(corner0[2:] + corner1[2:] + corner2[2:] + corner3[2:])
    ...
    try:
        ...
        segments_array = np.array(segment_list).reshape([-1, 4, 2])
        ...
        for connects, segments, square, degree in zip(connect_array, segments_array, squares, squares_degree):
            ...
            for start_idx in range(4):
                ...
                start_position, start_min, \
                start_cover_param, \
                start_peri_param = check_outside_inside(start_segments, connect_idx)
                end_position, end_min, \
                end_cover_param, \
                end_peri_param = check_outside_inside(end_segments, connect_idx)

                cover     += dist_segments[connect_idx] + start_cover_param * start_min + end_cover_param * end_min
                perimeter += dist_segments[connect_idx] + start_peri_param  * start_min + end_peri_param  * end_min

                square_length.append(
                    dist_segments[connect_idx] + start_peri_param * start_min + \
