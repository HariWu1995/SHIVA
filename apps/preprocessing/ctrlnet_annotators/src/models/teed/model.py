...
class DoubleFusion(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None):
        ...
        self.DWconv1 = nn.Conv2d(in_channels, in_channels*8, kernel_size=3, stride=1, padding=1, groups=in_channels)
    ...
    def forward(self, x):
        # fusecat = torch.cat(x, dim=1)
        attn1 = self.PSconv1(self.DWconv1(self.AF(x)))          # TEED best res TEDv14 [8, 32, 352, 352]
        attn2 = self.PSconv1(self.DWconv2(self.AF(attn1)))      # TEED best res TEDv14 [8, 3, 352, 352]
        return Fsmish(((attn2 + attn1).sum(1)).unsqueeze(1))    # TED best res
...
class TED(nn.Module):
    ...
    def forward(self, x, single_test=False):
        ...
        # Block 3
        block_3_pre = self.pre_dense_3(block_2_down)            #   [8, 64, 88, 88] block 3 L connection
        ...
        block_cat = self.block_cat(block_cat)  # B x 1 x H x W DoubleFusion
