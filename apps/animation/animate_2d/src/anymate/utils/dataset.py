...
class SingleVideoDataset(Dataset):
    ...
    def create_video_chunks(self):
        # Create a list of frames separated by sample frames [(1,2,3), (4,5,6), ...]
...
class VideoFolderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            "pixel_values": normalize_input(video[0]), 
            "frames": video[0],
            "prompt_ids": prompt_ids, 
            "text_prompt": prompt, 
            'dataset': self.__getname__(),
...
class CachedDataset(Dataset):
    ...
    def get_files_list(self):
        tensors_list = [f"{self.cache_dir}/{x}" for x in os.listdir(self.cache_dir) 
                                                 if x.endswith('.pt')]
