...
class Conv3d(nn.Conv3d, LoRALayer):
    ...
    def forward(self, x: torch.Tensor):
        if self.r > 0 and not self.merged:
            return F.conv3d(
                x, 
                self.weight + torch.mean((self.lora_B @ self.lora_A).view(self.view_shape), dim=-2,  keepdim=True) * self.scaling, 
                self.bias, 
                self.stride, 
                self.padding, 
                self.dilation, 
...
def add_lora_to(
    ...
    for module, name, child_module in \
                        find_modules(model, ancestor_class=target_module, search_class=search_class):
