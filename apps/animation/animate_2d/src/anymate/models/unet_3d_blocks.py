...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def __init__(
        ...
        if resnet_groups is None:
            resnet_groups = min(in_channels // 4, 32)
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def __init__(
        ...
        if add_downsample:
            self.downsamplers = nn.ModuleList([
                Downsample2D(out_channels, use_conv=True, 
                            out_channels=out_channels, padding=downsample_padding, name="op")
            ])
        else:
    ...
    def forward(
        ...
        # TODO: attention mask is not used
        output_states = ()
