...
with open(ui.CSS_DIR / 'html_readable_style.css', 'r') as f:
    readable_css = f.read()

with open(ui.CSS_DIR / 'html_instruct_style.css', 'r') as f:
    instruct_css = f.read()

# Custom chat styles
chat_styles = {
        k.stem: open(k, 'r').read() 
    for k in ui.CSS_DIR.glob('chat_style-*.css')
}

# Handle styles that derive from other styles
for k in chat_styles:
    lines = chat_styles[k].split('\n')
    match = re.search(r'chat_style-([a-z\-]*)\.css', lines[0])
    if not match:
        continue
...
def replace_quotes(text):
    ...
    pattern = '|'.join(f'({re.escape(open_q)})(.*?)'\
                       f'({re.escape(close_q)})' for open_q, close_q in quote_pairs)
...
def generate_instruct_html(history):
    ...
    for i in range(len(history['visible'])):
        ...
        converted_visible = [
            convert_to_markdown_wrapped(entry, use_cache = i != (len(history['visible']) - 1)) 
