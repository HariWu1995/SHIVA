...
class QuadraticSamplingLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        transformed_logits = torch.where(
            ...
            + (s * self.smoothing_factor * diff**3) + max_logit,
            scores
        )
...
class TailFreeLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        sorted_logits, \
        sorted_indices = torch.sort(scores, descending=True)
...
class XTCLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, threshold: float, probability: float, filter_value: float = -float("Inf")):
        ...
        self.filter_value = filter_value
        self.special_token_ids = [shared.tokenizer.encode("\n")[-1]]
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for input_ids_row, scores_row in zip(input_ids, scores):
            ...
            for i in match_indices:
                ...
                # (at most to 50 to prevent exponent overflow at penalty calculation) 
                # (this cap also improves performance on worst case)
...
def get_logits_processor_patch(self, **kwargs):
    ...
    if generation_config.dry_multiplier is not None \
        ...
        warpers.append(
            DRYLogitsProcessor(
                    multiplier=generation_config.dry_multiplier,
                        ...
                        _range=generation_config.repetition_penalty_range,
                allowed_length=generation_config.dry_length,
             sequence_breakers=sequence_breakers,
            )
        )

    if generation_config.tfs is not None \
    and generation_config.tfs > 0.0 \
    and generation_config.tfs < 1.0:
        warpers_to_add.append(
    ...
    if generation_config.top_a is not None \
    and generation_config.top_a > 0.0 \
    and generation_config.top_a < 1.0:
        warpers_to_add.append(
            TopALogitsWarper(
    ...
    if generation_config.xtc_probability is not None \
    ...
    if generation_config.mirostat_mode is not None \
    ...
    class_name_to_nickname = {
        ...
        'EpsilonLogitsWarper': 'eps_cutoff',
    ...
    def custom_sort_key(obj):
        ...
        if class_name not in class_name_to_nickname \
    ...
    if shared.verbose:
        logger.info("WARPERS=")
        pprinter.pprint([x.__class__.__name__ for x in warpers])
...
def generation_config_init_patch(self, **kwargs):
    ...
    self.dry_length    = kwargs.pop("dry_length", 2)
    ...
    self.sampler_priority = kwargs.pop("sampler_priority", [
                                ...
                                'top_n_sigma', 'top_k', 'top_p', 'typical_p', 'eps_cutoff', 'eta_cutoff', 
