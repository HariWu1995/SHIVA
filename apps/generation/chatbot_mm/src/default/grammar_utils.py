...
def remove_leading_white_space(src, newline_ok):
    ...
    This function processes the input string, skipping over any spaces, tabs,
        and content following a '#' character, which denotes a comment. 
    The parsing of a comment continues until the end of the line 
        (denoted by newline characters '\r' or '\n'). 
    If the 'newline_ok' parameter is set to False, 
        the function will stop processing and 
        return the remaining string upon encountering a newline character, 
        otherwise it will skip over newline characters as well.
    ...
    while grammar_encoding[pos]:
        ...
        while grammar_encoding[pos]:
            if grammar_encoding[pos] == REF_RULE_MARKER:
                ...
                print(f"<{pos}>{symbol_id_names[ref_rule_id]}", end=" ", file=file)
                pos += 2
            else:
...
class GrammarConstraint(ABC):
    ...
    # The parsing is a top-down, left-to-right, depth-first traversal of the grammar.
...
class IncrementalGrammarConstraint(GrammarConstraint):
    ...
    def accept_token_id(self, token_id: int, stacks: List[List[int]]):
        if token_id == self.eos_token_id:
            if stacks and all(len(stack) != 0 for stack in stacks):
                raise Exception(
                    f"At least 1 of stack should be empty when EOS is reached. However, stacks are {stacks}"
                )
            return []
    ...
    # If the grammar has an exceedingly large number of states, 
    #    the correct setting is a tradeoff between GPU RAM usage and recomputation time.
    # The main variable that pushes usage up here is number of states in the grammar.
    @lru_cache(maxsize=32768)
...
class TokenTrie:
    ...
    def load_tokens(self, tokenizer):
        ...
        if "gpt2" in tokenizer.__class__.__name__.lower():
            ...
            # Here, decoder does a string replace on a bunch of sequences like ' .' for '.'.
            # This interferes with assumption that a token should always have exactly 1 representation.
            # FIXME: text-generation-inference doesn't seem to run this cleanup, so we get extraneous spaces.
            # So, in order to generate the right token set for TGI, we have to skip the space trimming.
            # See: https://github.com/huggingface/transformers/blob/main/src/transformers/tokenization_utils_base.py#L3588-L3600
            def fmt_token(id):
                if id in special:
                    return None
            ...
            logger.warn("unrecognized tokenizer: using default token formatting")
