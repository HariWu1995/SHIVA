...
from markdown.blockprocessors import ListIndentProcessor, OListProcessor, ParagraphProcessor
    ...
    def get_level(self, parent: etree.Element, block: str) -> tuple[int, etree.Element]:
        ...
        while indent_level > level:
            ...
            if (child is not None and
               (child.tag in self.LIST_TYPES or 
                child.tag in self.ITEM_TYPES)):
...
class SaneOListProcessor(OListProcessor):
    ...
    """
    SIBLING_TAGS = ['ol']       # Exclude `ul` from list of siblings
    LAZY_OL = False             # Disable lazy list behavior
    ...
    def __init__(self, parser: blockparser.BlockParser):
        ...
        self.INDENT_RE = re.compile(r'^[ ]{%d,%d}[\*_]{0,2}((\d+\.)|[*+-])[ ]+.*' % (MIN_NESTED_LIST_INDENT, self.tab_length * 2 - 1))
    ...
    def run(self, parent: etree.Element, blocks: list[str]) -> None:
        ...
        # Loop through items in block, recursively parsing each with appropriate parent.
        for item in items:
...
class SaneUListProcessor(SaneOListProcessor):
    ...
    """
    TAG: str = 'ul'
    SIBLING_TAGS = ['ul']       # Exclude `ol` from list of siblings
