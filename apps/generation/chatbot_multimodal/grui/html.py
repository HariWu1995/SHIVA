...
for k in get_available_chat_styles():
    chat_styles[k] = open(CSS_DIR / f'chat_style-{k}.css', 'r').read()
...
def get_image_cache(path):
    ...
    if (path in image_cache and mtime != image_cache[path][0]) \
    or (path not in image_cache):
        img = make_thumbnail(Image.open(path))

        old_p = Path(f'{cache_folder}/{path.name}_cache.png')
        new_p = Path(f'{cache_folder}/cache_{path.name}.png')
        if old_p.exists():
            old_p.rename(new_p)
