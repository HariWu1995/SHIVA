...
extension_module = "apps.generation.chatbot_multimodal.grui.extensions.{name}.script"
...
def apply_settings(extension, name):
    ...
    for param in extension.params:
        _id = f"{name}-{param}"
        ui.default_settings[_id] = extension.params[param]
        if _id in ui.settings:
            extension.params[param] = ui.settings[_id]


def load_extensions():
    global state, setup_called
    state = {}
    for i, name in enumerate(ui.extensions):
        ...
        try:
            try:
                extension = import_module(extension_module.format(name))
...
def _apply_string_extensions(function_name, text, state, is_chat=False):
    ...
    for extension, _ in iterator():
        ...
        # Handle old extensions without the 'state' arg or  the 'is_chat' kwarg
        count = 0
        has_chat = False
...
def create_extensions_block():
    ...
    for extension, name in iterator():
        ...
        not (hasattr(extension, 'params') and \
