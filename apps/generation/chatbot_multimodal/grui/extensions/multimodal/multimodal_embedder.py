...
from grui.extensions.multimodal.pipeline_loader import load_pipeline

from src import shared
from src.generation import get_max_prompt_length, encode
...
class MultimodalEmbedder:
    ...
    def _split_prompt(self, prompt: str, load_images: bool = False) -> List[PromptPart]:
        ...
        It will also append `image_start` and `image_end` before and after the image, 
        and optionally parse and load the images, if `load_images` is `True`.
        """
    ...
    def _encode_single_text(self, part: PromptPart, add_bos_token: bool) -> PromptPart:
        ...
        """
        if part.is_image:
            placeholders = torch.ones((self.pipeline.num_image_embeds())) \
                                     * self.pipeline.placeholder_token_id()
        else:
            placeholders = encode(part.text, add_bos_token=add_bos_token)[0]
        part.input_ids = placeholders.to(device=shared.model.device, dtype=torch.int64)
        return part
    ...
    def _encode_text(self, state, parts: List[PromptPart]) -> List[PromptPart]:
        ...
        The chat/instruct mode should make prompts that fit in get_max_prompt_length, 
        but if max_new_tokens are set such that the context + min_rows don't fit, 
        we can get a prompt which is too long.
        We can't truncate image embeddings, as it leads to broken generation, 
        so remove the images instead and warn the user
        """
