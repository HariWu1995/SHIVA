...
def llama_cpp_lib(loader_config):
    ...
    for arg, lib_name in lib_names:
        ...
        should_import = (arg is None or loader_config)
        if should_import:
            if imported_module \
            and imported_module != lib_name:
            ...
            try:
                ...
                monkey_patch_llama_cpp_python(return_lib, loader_config)
                return return_lib
...
def monkey_patch_llama_cpp_python(lib, loader_config):
    ...
    def my_generate(self, *args, **kwargs):
        if loader_config.streaming_llm:
            new_sequence = args[0]
    ...
    if hasattr(lib, 'llama_chat_format') \
    and hasattr(lib.llama_chat_format, 'Jinja2ChatFormatter'):
