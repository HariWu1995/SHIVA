...
def process_llamacpp_cache(loader_config, model, new_sequence, past_sequence):

    if len(past_sequence) == 0 \
    or len(new_sequence) == 0:
    ...
    if i1 > 0 and overlap_length > 0.2 * len(new_sequence):
        ...
        sink_length = max(prefix_length, loader_config.attention_sink_size)
...
def find_prefix_length(past_seq, tensor_seq):
    '''
...
def find_longest_common_substring_indices(list1, list2):
    '''
    Given 2 lists, solves the Longest Common Substring problem.
    ...
    for index1 in range(0, len_list1):
        ...
        while index2 >= 0:
            ...
            while temp_index1 < len_list1 \
              and temp_index2 < len_list2 \
              and list2[temp_index2] == list1[temp_index1]:
    ...
    return (
        start_index_list1, end_index_list1, 
        start_index_list2, end_index_list2,
    )
