...
from .sampler import loaders_samplers
...
def load_preset(name, verbose=False):
    ...
    if verbose:
        ...
        pprinter.pprint(remove_defaults(generate_params))
...
def random_preset(state):
    params_and_values = {
        ...
        'flatten_distribution': {
            ...
            'dynamic_temperature': [
                [0.1, 1], [0.1, 1.5], [0.1, 2], [0.1, 5],
                [0.5, 1], [0.5, 1.5], [0.5, 2], [0.5, 5],
                [0.8, 1], [0.8, 1.5], [0.8, 2], [0.8, 5],
                            [1, 1.5],   [1, 2],   [1, 5],
            ],
    ...
    pprinter.pprint(remove_defaults(state))
    return state, *[generate_params[k] for k in presets_params()]


def loader_contains(sampler):
    if sampler == 'dynamic_temperature' \
    and 'dynatemp_low' in loaders_samplers[shared.loader]:
        return True
    else:
        return sampler in loaders_samplers[shared.loader]


def remove_defaults(state):
    defaults = default_preset()
    data = {
        k: state[k] for k in presets_params()
    }
