...
class MP3Ddataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        images_raw = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in images_raw]
        ...
        for i in range(num_views):
            ...
            img = warp_img(90, _degree, 0, images_raw, self.vx, self.vy)
            img = cv2.resize(img, (self.resolution, self.resolution))

            K, R = get_K_R(90, _degree, 0, self.resolution, self.resolution)
        ...
        prompt = []
        prompt_dir = os.path.dirname(
                     self.data[idx][0].replace('matterport_skybox_images', 'blip3'))
        image_name = self.data[idx][0].split('/')[-1].split('_')[0]
        
        for i in range(num_views):
            _degree = (init_degree + i * self.rot) % 360
            _degree = int(np.round(_degree / 45) * 45) % 360
            txt_path = os.path.join(f'{image_name}_{_degree}.txt')
            prompt_path = os.path.join(self.image_root_dir, prompt_dir, txt_path)
            prompt.append('This is one view of a scene. ' + self.load_prompt(prompt_path))
