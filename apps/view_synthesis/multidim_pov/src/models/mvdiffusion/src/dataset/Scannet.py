...
class Scannetdataset(Dataset):
    ...
    def __init__(self, config, mode='train'):
        ...
        if len(scenes) == 0:
            raise Exception(f'No scenes in {scannet_root_dir}, please check ScanNet is in the correct path')
        else:
            print(f'Found {len(scenes)} scenes')

        for scene_id in sorted(scenes):
            scene_dir = os.path.join(scannet_root_dir, scene_id)
            valid_file = os.path.join(scene_dir, 'valid_frames.npy')
            if os.path.exists(valid_file):
                valid_id = np.load(valid_file)
            else:
                raise Exception(f'{scene_id} does not contain valid_frames.npy')
            
            if config['data_load_mode'] == 'fix_interval':
                max_id = max(valid_id)
                kf_ids = [i for i in range(0, max_id, config['test_interval']) if i in valid_id]
                # relex this number if using GPU with large memory
                if len(kf_ids) > 150:
                    continue
            else:
                kf_path = os.path.join(scene_dir, 'key_frame_0.6.txt')
                if not os.path.exists(kf_path):
                    raise Exception(f"{scene_id} does not contain key_frame_0.6.txt")
            ...
            for i, kf_id in enumerate(kf_ids):
                kf_rgb_path = os.path.join(scene_dir, f'color/{kf_id}.jpg')
                if not os.path.exists(kf_rgb_path):
                    continue
                prompt_path = kf_rgb_path.replace('color', 'prompt')\
                                         .replace('jpg', 'txt')
                if os.path.exists(prompt_path):
                    ...
                    if (i == 0) \
                    or (mode == 'train') \
                    or (config['data_load_mode'] == 'fix_frame' and i % config['num_views'] == 0):
    ...
    def _get_consecutive_kfs_inp(self, scene_id):
        ...
        for i in range(len(img_key_paths) - 1):
            ...
            while j * interval + idx1 < idx2:
                ...
                if idx in valid_ids:
                    img_paths.append(
                        os.path.join('/'.join(img_key_paths[0].split('/')[:-1]), 
                                     str(idx)+'.jpg')
                    )
                    masks.append(False)
                j += 1
        ...
        if idx_base + num_views < num_kfs:
            ...
            if idx_start < 0:
                ...
                while len(img_paths) < num_views:
                    ...
                    if idx in valid_ids:
                        img_paths.append(
                            os.path.join('/'.join(img_path.split('/')[:-1]), 
                                         str(idx)+'.jpg')
                        )   
            else:
    ...
    def load_seq(self, image_paths, resolution):
        images_ori = [
            cv2.cvtColor(cv2.imread(path)[..., :3], cv2.COLOR_BGR2RGB) 
        ...
        for p in image_paths:
            pose_path = p.replace('color', 'pose')\
                         .replace('jpg', 'txt')
            ...
            prompt_path = p.replace('color', 'prompt')\
                           .replace('jpg', 'txt')
        ...
        # load k
        k_path = os.path.join('/'.join(image_paths[0].replace('color', 'intrinsic').split('/')[:-1]), 
                              'intrinsic_depth.txt')
        k = np.loadtxt(k_path)
        ...
        # load depth
        depth_path = [p.replace('color', 'depth')\
                       .replace('jpg', 'png') for p in image_paths]
        ...
        depths = [cv2.resize(x, resolution, interpolation=cv2.INTER_NEAREST) for x in depths]
        depths = np.stack(depths, axis=0)

        depth_valid_mask = depths > 0
        depth_inv = 1. / (depths + 1e-6)
        depth_max = [depth_inv[i][depth_valid_mask[i]].max() for i in range(depth_inv.shape[0])]
        depth_min = [depth_inv[i][depth_valid_mask[i]].min() for i in range(depth_inv.shape[0])]
    ...
    def __getitem__(self, idx):
        ...
        if self.data_load_mode == 'fix_interval':
            ...
            if self.mode == 'train' and p_rand > self.gen_data_ratio:
                image_paths, mask = self._get_consecutive_kfs_inp(scene_id, img_path)
            else:
                image_paths = self._get_consecutive_kfs_fix_frame(scene_id, img_path)
                mask = np.ones(len(image_paths)).astype(np.bool)

        elif self.data_load_mode == 'two_stage':
            image_paths, mask = self._get_consecutive_kfs_inp(scene_id, img_path)

        images, depths, depth_inv_norm, \
                poses, K, prompts = self.load_seq(image_paths, self.resolution)
        
        depth_inv_norm_small = np.stack([
            cv2.resize(depth_inv_norm[i], (self.resolution[0] // 8, 
                                           self.resolution[1] // 8), 
