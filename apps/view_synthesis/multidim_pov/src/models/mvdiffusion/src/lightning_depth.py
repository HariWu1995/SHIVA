...
class DepthGenerator(pl.LightningModule):
    ...
    def encode_image(self, x_input):
        ...
        z = z.sample()
        z = z.reshape(b, -1, z.shape[-3], z.shape[-2], z.shape[-1])
        z = z * 0.18215
        return z
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            'scheduler': lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.max_epochs, eta_min=1e-7),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.trainer.global_rank == 0:
            self.save_image(images_pred, images, batch['prompt'][0], 
    ...
    def inference_inp(self, batch):
        ...
        for i, t in enumerate(timesteps):
            ...
            noise_pred = self.forward_cls_free(latents, _timestep, prompt_embd, batch, 
                                               self.mv_base_model, type='interpolation')
    ...
    def inference_gen(self, batch):
        ...
        for i, t in enumerate(timesteps):
            _timestep = torch.cat([t[None, None]] * m, dim=1)
            noise_pred = self.forward_cls_free(latents, _timestep, prompt_embd, batch, 
                                               self.mv_base_model, type='generation')
            latents = self.scheduler.step(noise_pred, t, latents).prev_sample
    ...
    def test_step(self, batch, batch_idx):  
        ...
        output_dir = os.path.join(self.logger.log_dir, 'images', '{}_{}'.format(scene_name, key_id))
        os.makedirs(output_dir, exist_ok=True)
        
        images = ((batch['images'] + 1) * 127.5).cpu().numpy().astype(np.uint8)
        depths = ( batch['depths'] * 1_000     ).cpu().numpy().astype(np.uint16)
        poses  = ( batch['poses']              ).cpu().numpy().astype(np.float32)
        depth_inv = (batch['depth_inv_norm']   ).cpu().numpy().astype(np.float32)
        ...
        for i, path in enumerate(image_paths):
            ...
            image_pred_path = os.path.join(output_dir, f'{image_id}_pred.png')
            image_gt_path   = os.path.join(output_dir, f'{image_id}_gt.png')

            Image.fromarray(images_pred[0, i]).save(image_pred_path)
            Image.fromarray(images     [0, i]).save(image_gt_path)

            depth_path = os.path.join(output_dir, f'{image_id}_depth.png')
            poses_path = os.path.join(output_dir, f'{image_id}_poses.txt')

            cv2.imwrite(depth_path, depths[0, i])
            np.savetxt(poses_path, poses[0, i])

            depth_inv_path = os.path.join(output_dir, f'{image_id}_depth_inv.png')
            plt.imsave(depth_inv_path, depth_inv[0, i])
    ...
    def save_image(self, images_pred, images, prompt, depth_inv_full, batch_idx):
        ...
        if images_pred is not None:
            for m_i in range(images_pred.shape[1]):
                ...
                im.save(
                    os.path.join(img_dir, f'{self.global_step}_{batch_idx}_{m_i}_pred.png')
                )
        ...
        for m_i in range(images.shape[1]):
            ...
            plt.imsave(
                os.path.join(img_dir, f'{self.global_step}_{batch_idx}_{m_i}_depth_inv.png'), 
