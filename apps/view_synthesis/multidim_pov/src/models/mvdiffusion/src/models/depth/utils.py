...
def get_key_value(
    ...
    mask_proj = []
    kernal_size = 1
    depth_query = depth_query[:, query_scale//2::query_scale, 
    ...
    for i in range(0 - kernal_size//2, 1 + kernal_size//2):
        for j in range(0 - kernal_size//2, 1 + kernal_size//2):
            ...
            depth_i = torch.stack([
                depths[b_i, xy_l_round[b_i, ..., 1], 
                            xy_l_round[b_i, ..., 0]] for b_i in range(b)
            ])

            mask = mask * (depth_i > 0)
            depth_i[~mask] = 1_000_000
    ...
    xy = torch.tensor(xy, device=key_value.device).float()[None].repeat(xy_proj.shape[0], 1, 1, 1)   
