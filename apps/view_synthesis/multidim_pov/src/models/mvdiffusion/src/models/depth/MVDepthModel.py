...
class MultiViewBaseModel(nn.Module):
    ...
    def __init__(self, unet, config):
        ...
        for i in range(len(self.unet.down_blocks)):
            self.cp_blocks_encoder.append(
                CPBlock(self.unet.down_blocks[i].resnets[-1].out_channels, flag360=False)
            )
        ...
        for i in range(len(self.unet.up_blocks)):
            self.cp_blocks_decoder.append(
                CPBlock(self.unet.up_blocks[i].resnets[-1].out_channels, flag360=False)
            )

        self.condition_conv_in = nn.Conv2d(in_channels=5, out_channels=self.unet.conv_in.out_channels, kernel_size=3, stride=1, padding=1)
        ...
        self.trainable_parameters += [(
            list(self.condition_conv_in.parameters()) +
            list(self.condition_downblocks.parameters()) +
            list(self.condition_upblocks.parameters()), 
            1.0
        )]
    ...
    def get_correspondence(self, cp_package):
        ...
        for i in range(m):
            ...
            point_ij, _ = get_correspondence(depth_i, pose_rel, K, None)  # bs, 2, hw
            point_ij = rearrange(point_ij, '(b m) h w c -> b m h w c', b=b)
            
            correspondence[:, i] = point_ij
            
            mask = (point_ij[:, :, :, :, 0] >= 0) & (point_ij[:, :, :, :, 0] < w) & \
                   (point_ij[:, :, :, :, 1] >= 0) & (point_ij[:, :, :, :, 1] < h)
        ...
        for b_i in range(b):
            for i in range(m):
                for j in range(i+1, m):
                    overlap_ratios[b_i, i, j] = \
                    overlap_ratios[b_i, j, i] = min(overlap_ratios[b_i, i, j], 
                                                    overlap_ratios[b_i, j, i])

        # filter image pairs that have too small overlaps
        overlap_mask = overlap_ratios > self.overlap_filter
    ...
    def forward(self, latents_lr, timestep, prompt_embd, meta):
        ...
        for i, downsample_block in enumerate(self.unet.down_blocks):
            ...
            if hasattr(downsample_block, 'has_cross_attention') \
                ...
                for resnet, attn in zip(downsample_block.resnets, 
                                        downsample_block.attentions):

                    hidden_states = resnet(hidden_states, emb)
                    hidden_states = attn(hidden_states, encoder_hidden_states=prompt_embd).sample
            ...
            if m > 1:
                hidden_states = self.cp_blocks_encoder[i](hidden_states, reso_lr, meta, m)
        ...
        for attn, resnet in zip(self.unet.mid_block.attentions, 
                                self.unet.mid_block.resnets[1:]):
            hidden_states = attn(hidden_states, encoder_hidden_states=prompt_embd).sample
            hidden_states = resnet(hidden_states, emb)

        # correspondence aware attention
        if m > 1:
            hidden_states = self.cp_blocks_mid(hidden_states, reso_lr, meta, m)
        ...
        for i, upsample_block in enumerate(self.unet.up_blocks):
            ...
            if hasattr(upsample_block, 'has_cross_attention') \
                ...
                for resnet, attn in zip(upsample_block.resnets, 
            ...
            if m > 1:
                hidden_states = self.cp_blocks_decoder[i](hidden_states, reso_lr, meta, m)
