...
def get_correspondences(R, K, img_h, img_w):
    ...
    for i in range(m):  
        for j in range(m):
            ...
            homo_l = (
                K_right @ torch.inverse(R_right) @
                R_left  @ torch.inverse(K_left)
            )

            xyz_l = torch.tensor(get_x_2d(img_w, img_h), device=R.device)
            xyz_l = (xyz_l.reshape(-1, 3).T)[None].repeat(homo_l.shape[0], 1, 1)
            xyz_l = homo_l @ xyz_l

            xy_l = (xyz_l[:, :2] / xyz_l[:, 2:]).permute(0, 2, 1)\
                                                .reshape(-1, l, img_h, img_w, 2)
...
def get_key_value(key_value, xy_l, homo_r, ori_h, ori_w, ori_h_r, query_h):
    ...
    xy_proj_back = torch.cat([
        xy_proj, torch.ones(*xy_proj.shape[:-1], 1, device=xy_proj.device)], dim=-1)
    ...
    xy = xy[query_scale//2::query_scale, 
...
def get_query_value(query, key_value, xy_l, homo_r, img_h_l, img_w_l, img_h_r=None, img_w_r=None):
    ...
    for i in range(m):
        ...
        _key_value, _xy, _mask = get_key_value(key_value[:, i], 
                                                    xy_l[:, i], 
                                                  homo_r[:, i],
                                                    img_h_l, 
                                                    img_w_l, 
