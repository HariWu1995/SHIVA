    ...
    def forward(self, x, reso, cp_package, m):
        ...
        for b_i in range(b):
            ...
            for i in range(m):
                ...
                if len(indexs) == 0: 
                    # if the image does not have overlap with others, use the nearby images
                    if i == 0:
                        indexs = [1]
                    elif i == m-1:
                        indexs = [m-2]
                ...
                query, key_value, key_value_xy, \
                    mask = get_query_value(x_left, x_right, xy_l, xy_r, 
                                           depth_query, _depths, pose_rel, _K, 
                ...
                query_pe = self.pe(
                            torch.zeros(query.shape[0], 1, 1, device=query.device))
