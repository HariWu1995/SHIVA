    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: t.unsqueeze(3)
                       .reshape(B, t.shape[1], 1, C).permute(0, 2, 1, 3)
        ...
        out = (
            out.unsqueeze(0)
               .reshape(B, 1, out.shape[1], C).permute(0, 2, 1, 3)
...
def make_attn(in_channels, attn_type="vanilla", attn_kwargs=None):
    assert attn_type in ["vanilla", "vanilla-xformers", "memory-efficient-cross-attn", "linear", "none"], \
...
class Encoder(nn.Module):
    ...
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # end
        self.norm_out = Normalize(block_in)
        self.conv_out = nn.Conv2d(block_in, 2*z_channels if double_z \
                                        else z_channels, kernel_size=3, stride=1, padding=1)
    ...
    def __init__(self, in_channels, out_channels, *args, **kwargs):
        ...
        self.model = nn.ModuleList([
            ...
            ResnetBlock(in_channels=in_channels,
                        out_channels=2 * in_channels, temb_channels=0, dropout=0.0),
            ResnetBlock(in_channels=2 * in_channels,
                        out_channels=4 * in_channels, temb_channels=0, dropout=0.0),
...
class LatentRescaler(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.res_block1:
            x = block(x, None)
        x = F.interpolate(x, size=(int(round(x.shape[2]*self.factor)), 
...
class MergedRescaleEncoder(nn.Module):
    ...
    def __init__(self, in_channels, ch, resolution, out_ch, num_res_blocks,
        ...
        self.encoder = Encoder(in_channels=in_channels, num_res_blocks=num_res_blocks, ch=ch, ch_mult=ch_mult,
                               ...
                               attn_resolutions=attn_resolutions, dropout=dropout, resamp_with_conv=resamp_with_conv, out_ch=None)
...
class MergedRescaleDecoder(nn.Module):
    ...
    def __init__(self, z_channels, out_ch, resolution, 
                        num_res_blocks, attn_resolutions, ch, ch_mult=(1,2,4,8),
