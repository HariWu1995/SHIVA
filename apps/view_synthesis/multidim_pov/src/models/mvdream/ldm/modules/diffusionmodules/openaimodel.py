...
class Downsample(nn.Module):
    ...
    :param dims: determines if the signal is 1D, 2D, or 3D. 
                 If 3D, then downsampling occurs in the inner-two dimensions.
    """
    def __init__(self, channels, use_conv, dims=2, out_channels=None,padding=1):
        super().__init__()
        self.channels = channels
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO: check checkpoint usage, is True 
        # TODO: fix the .half call!!!
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim), nn.SiLU(),
        ...
        if self.num_classes is not None:
            if isinstance(self.num_classes, int):
                ...
                self.label_emb = nn.Sequential(
                    nn.Sequential(
                        linear(adm_in_channels, time_embed_dim), nn.SiLU(),
                        linear(time_embed_dim, time_embed_dim),
                    )
                )
            else:
    ...
    def __init__(
        ...
        if camera_dim is not None:
            ...
            self.camera_embed = nn.Sequential(
                linear(    camera_dim, time_embed_dim), nn.SiLU(),
                linear(time_embed_dim, time_embed_dim),
            )
