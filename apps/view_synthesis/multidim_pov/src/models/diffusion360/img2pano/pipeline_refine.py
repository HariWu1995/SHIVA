...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from txt2panoimage.pipeline_sr import StableDiffusionControlNetImg2PanoPipeline
        >>> base_model_path = "models/sr-base"
        >>> controlnet_path = "models/sr-control"
        >>> controlnet = ControlNetModel.from_pretrained(controlnet_path, torch_dtype=torch.float16)
        >>> pipe = StableDiffusionControlNetImg2PanoPipeline.from_pretrained(base_model_path, controlnet=controlnet, torch_dtype=torch.float16)
...
class StableDiffusionControlNetImg2PanoPipeline(
    ...
    def check_inputs(
        ...
        # Check `image`
        is_compiled = hasattr(F, 'scaled_dot_product_attention') \
                and isinstance(self.controlnet, torch._dynamo.eval_frame.OptimizedModule)
        ...
        if negative_prompt_embeds is None:
            ...
            if batch_size != len(negative_prompt):
                raise ValueError(
                    ...
                    'Please make sure that `negative_prompt` matches the batch size of `prompt`.'
                )
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                condition_warm = (i + 1) > num_warmup_steps \
