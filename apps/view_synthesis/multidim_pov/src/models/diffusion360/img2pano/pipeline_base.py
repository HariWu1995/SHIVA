...
from diffusers.utils import (
    PIL_INTERPOLATION, logging, deprecate, 
...
class StableDiffusionImage2PanoPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __init__(
        ...
        if safety_checker is not None and feature_extractor is None:
            raise ValueError(
                ...
                "If you do not want to use the safety checker, you can pass `'safety_checker=None'` instead."
            )
    ...
    def check_inputs(
        ...
        if (
            isinstance(self.controlnet, ControlNetModel) or is_compiled
        and isinstance(self.controlnet._orig_mod, ControlNetModel)
        ):
            self.check_image(image, prompt, prompt_embeds)
        elif (
            isinstance(self.controlnet, MultiControlNetModel) or is_compiled
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
        ...
        if isinstance(generator, list):
            init_latents = [self.vae.encode(image[i : i + 1]).latent_dist.sample(generator[i]) 
...
negative_prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        def controlnet_forward(
            ...
            if self.config.global_pool_conditions:
                down_block_res_samples = [torch.mean(sample, dim=(2, 3), keepdim=True) 
        ...
        global_pool_conditions = (
                controlnet.config.global_pool_conditions if isinstance(controlnet, ControlNetModel)
        ...
        control_image = torch.cat([control_image, 
                                   control_image[:, :, :, :self.blend_extend * self.vae_scale_factor]], dim=-1)
        if mask is not None:
            mask = torch.cat([mask] * batch_size, dim=0)
        ...
        # let's offload unet and controlnet manually for max memory savings
