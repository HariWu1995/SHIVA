...
def pad_tokens_and_weights(tokens, weights, max_length,
                           bos, eos, pad, no_boseos_middle=True, chunk_length=77):
    r"""
        ...
        if no_boseos_middle:
            ...
            if len(weights[i]) == 0:
                ...
                for j in range(max_embeddings_multiples):
                    ...
                    w += weights[i][ j      * (chunk_length - 2):min(len(weights[i]), 
                                    (j + 1) * (chunk_length - 2))]
...
def get_weighted_text_embeddings(
    ...
    if not skip_parsing:
        prompt_tokens, \
        prompt_weights = get_prompts_with_weights(pipe, prompt, max_length - 2)
        if uncond_prompt is not None:
            if isinstance(uncond_prompt, str):
                uncond_prompt = [uncond_prompt]
            uncond_tokens, \
            uncond_weights = get_prompts_with_weights(pipe, uncond_prompt, max_length - 2)
    else:
        prompt_tokens = [
                token[1:-1] 
...
class StableDiffusionControlNetImg2ImgPanoPipeline(StableDiffusionControlNetImg2ImgPipeline):
    ...
    def check_inputs(
        ...
        if prompt_embeds is not None and negative_prompt_embeds is not None:
            if prompt_embeds.shape != negative_prompt_embeds.shape:
                raise ValueError(
                    '`prompt_embeds` and `negative_prompt_embeds` must have the same shape when passed directly, but got: '
                    f'`prompt_embeds` {prompt_embeds.shape} != `negative_prompt_embeds` {negative_prompt_embeds.shape}.'
                )

        # `prompt` needs more sophisticated handling when there are multiple conditionings.
        if isinstance(self.controlnet, MultiControlNetModel):
            if isinstance(prompt, list):
                logger.warning(
                    f'You have {len(self.controlnet.nets)} ControlNets and you have passed {len(prompt)} prompts.'
                    ' The conditionings will be fixed across the prompts.'
                )

        # Check `image`
        is_compiled = hasattr(F, 'scaled_dot_product_attention') and \
            isinstance(self.controlnet, torch._dynamo.eval_frame.OptimizedModule)
          ...
          and isinstance(self.controlnet._orig_mod, MultiControlNetModel)):
            if isinstance(controlnet_conditioning_scale, list):
                if any(isinstance(i, list) for i in controlnet_conditioning_scale):
                    ...
                    and len(controlnet_conditioning_scale) != len(self.controlnet.nets):
                raise ValueError(
    ...
    def _default_height_width(self, height, width, image):
        # NOTE: It is possible that a list of images have different dimensions for each image, 
        # so just checking the first image is not _exactly_ correct, but it is simple.
    ...
    def denoise_latents(self, latents, t, prompt_embeds, control_image,
        ...
        if guess_mode and do_classifier_free_guidance:
            ...
            mid_block_res_sample = torch.cat([torch.zeros_like(mid_block_res_sample), 
    ...
    def __call__(
        ...
        controlnet = self.controlnet._orig_mod if is_compiled_module(self.controlnet) else \
                     self.controlnet
        ...
        global_pool_conditions = (
            controlnet.config.global_pool_conditions if isinstance(controlnet, ControlNetModel) else
        ...
        timesteps, \
    num_inference_steps = self.get_timesteps(num_inference_steps, strength, device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                rows = []
                for latents_input_, \
                control_image_input_ in zip(latents_input, control_image_input):
                    ...
                    control_image_input_ = torch.cat(
                        [x[0, :, :, ][None, :, :, :] for x in control_image_input_[:num_block]] \
                      + [x[1, :, :, ][None, :, :, :] for x in control_image_input_[:num_block]],
                        dim=0
                    )
