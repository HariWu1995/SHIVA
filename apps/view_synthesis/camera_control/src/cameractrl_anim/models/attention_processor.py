...
class AttnProcessor:
    ...
    def __call__(
        ...
        if input_ndim == 4:
            B, channel, H, W = hidden_states.shape
            hidden_states = hidden_states.view(B, channel, H * W).transpose(1, 2)

        B, seq_len, _ = hidden_states.shape if encoder_hidden_states is None \
        ...
        if input_ndim == 4:
            hidden_states = hidden_states.transpose(-1, -2).reshape(B, channel, H, W)
