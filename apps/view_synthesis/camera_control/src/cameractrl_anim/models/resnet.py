    ...
    def __init__(
        ...
        *,
        ...
        if self.use_in_shortcut:
            self.conv_shortcut = nn.Conv2d(in_channels, conv_2d_out_channels, 
                                        kernel_size=1, stride=1, padding=0, bias=conv_shortcut_bias)
        else:
            self.conv_shortcut = None

        conv_out = nn.Conv2d(conv_2d_out_channels, conv_2d_out_channels, kernel_size=1, stride=1, padding=0)
...
class Upsample3D(nn.Module):
    ...
    def __init__(self, in_channels, use_conv=False, use_conv_transpose=False, out_channels=None, name="conv"):
        super().__init__()
        self.in_channels = in_channels
        ...
        if use_conv_transpose:
            ...
            self.conv = InflatedConv3d(self.in_channels, self.out_channels, 3, padding=1)
...
class Downsample3D(nn.Module):
    ...
    def __init__(self, in_channels, use_conv=False, out_channels=None, padding=1, name="conv"):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels or in_channels
        self.use_conv = use_conv
        ...
        stride = 2
        if use_conv:
            self.conv = InflatedConv3d(self.in_channels, self.out_channels, 3, stride=stride, padding=padding)
        else:
            raise NotImplementedError
...
class ResnetBlock3D(nn.Module):
    ...
    def forward(self, input_tensor, temb):
        ...
        time_len = hidden_states.shape[2]
        emb = repeat(emb, "b c -> (b f) c", f=time_len)
...
class Mish(nn.Module):
    ...
    def forward(self, hidden_states):
        return hidden_states * torch.tanh(F.softplus(hidden_states))
