...
from diffusers.models.attention_processor import AttentionProcessor, LoRAAttnProcessor

from .resnet import InflatedConv3d, FusionBlock2D
...
from .attention_processor import (
    LORAPoseAdaptorAttnProcessor, LoRAAttnProcessor as CustomizedLoRAAttnProcessor,
        PoseAdaptorAttnProcessor,     AttnProcessor as CustomizedAttnProcessor,
)
...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        for downsample_block, down_fuser, \
            motion_module_alpha in zip(self.down_blocks, self.down_fusers[1:], motion_module_alphas[:-1]):
        ...
        if down_block_additional_residuals is not None:
            ...
            for down_block_res_sample, \
                down_block_additional_residual in zip(down_block_res_samples, 
                ...
                if len(down_block_additional_residual.shape) == 4:
                    ...
                    # broadcast will solve the problem
        ...
        if mid_block_additional_residual is not None:
            if len(mid_block_additional_residual.shape) == 4:
                ...
                # broadcast will solve this problemq
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None, logger=None):
        ...
        config["down_block_types"] = ["CrossAttnDownBlock3D"] * 3 + ["DownBlock3D"]
        config["up_block_types"] = ["UpBlock3D"] + ["CrossAttnUpBlock3D"] * 3
        ...
        if (logger is not None) and (len(unused_kwargs) > 0):
...
class UNet3DConditionModelPoseCond(UNet3DConditionModel):
    ...
    def extract_init_dict(cls, config_dict, **kwargs):
        ...
        if len(config_dict) > 0:
            print(
                f"The config attributes {config_dict} were passed to {cls.__name__}, but are "
                f"not expected and will be ignored. Please verify your {cls.config_name} configuration file."
            )
    ...
    def set_all_attn_processor(
        ...
        **attention_processor_kwargs
        ...
        spatial_lora_rank = spatial_lora_kwargs.pop('lora_rank')
        if spatial_lora_rank <= 16:
            spatial_lora_rank = hidden_size // spatial_lora_rank
    ...
    def forward(
        ...
        for downsample_block, pose_embedding_feature, \
            down_fuser, motion_module_alpha in zip(self.down_blocks, pose_embedding_features,
                                                   self.down_fusers[1:], motion_module_alphas[:-1]):
