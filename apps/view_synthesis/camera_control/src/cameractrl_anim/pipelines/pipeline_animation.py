...
class AnimationPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def __init__(
        ...
        if hasattr(scheduler.config, "steps_offset") \
            ...
            deprecation_message = (
                ...
                f"should be set to 1 instead of {scheduler.config.steps_offset}. Please make sure to update "
                "the config accordingly as leaving `steps_offset` might led to incorrect results in future versions. "
                "If you have downloaded this checkpoint from the Hugging Face Hub, "
                "it would be very nice if you could open a Pull request for the `scheduler/scheduler_config.json` file"
            )
        ...
        if is_unet_version_less_0_9_0 \
        and is_unet_sample_size_less_64:
            deprecation_message = (
    ...
    def _encode_prompt(self, prompt, device, num_videos_per_prompt, do_classifier_free_guidance, negative_prompt):
        ...
        if text_input_ids.shape[-1] >= untruncated_ids.shape[-1] \
        ...
        if do_classifier_free_guidance:
            ...
            if negative_prompt is None:
                ...
                raise ValueError(
                    ...
                    f"Please make sure that passed `negative_prompt` matches the batch size of `prompt`."
                )
            else:
                uncond_tokens = negative_prompt

            max_length = text_input_ids.shape[-1]
            uncond_input = self.tokenizer(uncond_tokens, max_length=max_length, padding="max_length", 
    ...
    def prepare_latents(self, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None:
            ...
            if isinstance(generator, list):
                shape = shape
                # shape = (1,) + shape[1:]
                latents = [torch.randn(shape, generator=generator[i], device=rand_device, dtype=dtype)
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_full = torch.zeros_like(latents).to(latents.device)
                noise_preds = []
                ...
                for pred_idx, noise_pred in enumerate(noise_preds):
                    ...
                    noise_full[:, :, start_idx: start_idx + single_model_length] += noise_pred / mask_full[:, :, start_idx: start_idx + single_model_length]

                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_full, t, latents, **extra_step_kwargs).prev_sample
