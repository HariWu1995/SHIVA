...
def assign_to_checkpoint(
    paths, checkpoint, old_checkpoint, 
...
def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=False, controlnet=False):
    ...
    if config["class_embed_type"] is None:
        ...
        ...
    elif config["class_embed_type"] == "timestep" \
    ...
    num_input_blocks = len({".".join(layer.split(".")[:2]) 
    ...
    num_middle_blocks = len({".".join(layer.split(".")[:2]) 
    ...
    num_output_blocks = len({".".join(layer.split(".")[:2]) 
    ...
    for i in range(1, num_input_blocks):
        ...
        assign_to_checkpoint(
            paths, new_checkpoint, unet_state_dict, 
    ...
    meta_path = {
        "old": "middle_block.1", 
        "new": "mid_block.attentions.0",
    }
    assign_to_checkpoint(
        attentions_paths, new_checkpoint, unet_state_dict, 
        additional_replacements=[meta_path], config=config
    )
    ...
    for i in range(num_output_blocks):
        ...
        if len(output_block_list) > 1:
            ...
            output_block_list = {
                k: sorted(v) for k, v in output_block_list.items()
            }
...
def convert_ldm_vae_checkpoint(checkpoint, config):
    ...
    num_down_blocks = len({".".join(layer.split(".")[:3]) 
    ...
    for i in range(num_down_blocks):
        resnets = [
            ...
            if f"down.{i}" in key 
            and f"down.{i}.downsample" not in key
        ]
...
protected = {
    re.escape(x[0]): x[1] for x in textenc_transformer_conversion_lst
}
