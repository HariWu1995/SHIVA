...
from diffusers.models.unets.unet_3d_blocks import DownBlockSpatioTemporal, UpBlockSpatioTemporal
...
def get_down_block(
    ...
    **kwargs,
) -> Union["DownBlockSpatioTemporal", "CrossAttnDownBlockSpatioTemporalPoseCond"]:
...
def get_up_block(
    ...
    transformer_layers_per_block: int = 1,
    **kwargs,
) -> Union["UpBlockSpatioTemporal", "CrossAttnUpBlockSpatioTemporalPoseCond"]:
...
class CrossAttnDownBlockSpatioTemporalPoseCond(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            resnets.append(
                SpatioTemporalResBlock(in_channels=in_channels,
                                      out_channels=out_channels,
                                     temb_channels=temb_channels, eps=1e-6)
            )
            attentions.append(
        ...
        if add_downsample:
            self.downsamplers = nn.ModuleList([
                Downsample2D(out_channels, 
                            out_channels=out_channels, use_conv=True, padding=1, name="op")
            ])
        else:
    ...
    def forward(
        ...
        for resnet, attn in blocks:
            ...
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = checkpoint(create_custom_forward(resnet),
                                           hidden_states, temb, image_only_indicator, **ckpt_kwargs)
                ...
                hidden_states = resnet(hidden_states, temb,
                                        image_only_indicator=image_only_indicator)  # [bs * frame, c, h, w]
                hidden_states = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    image_only_indicator=image_only_indicator,
    ...
    def __init__(
        ...
        resnets = [
            SpatioTemporalResBlock(in_channels=in_channels,
                                 ...
                                 temb_channels=temb_channels, eps=1e-5)
        ]
        attentions = []
