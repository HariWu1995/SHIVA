...
from diffusers.utils import is_xformers_available, USE_PEFT_BACKEND
...
class AttnProcessor:
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            ...
            B, seq_len, _ = encoder_hidden_states.shape
...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        attention_mask = attn.prepare_attention_mask(attention_mask, key_tokens, B)
        if attention_mask is not None:
            # expand our mask's singleton query_tokens dimension:
            #       [batch * heads, 1, key_tokens] -> [batch * heads, query_tokens, key_tokens]
...
class PoseAdaptorAttnProcessor(nn.Module):
    ...
    def __call__(self, attn, hidden_states, pose_feature,
                     encoder_hidden_states=None, 
                            attention_mask=None, temb=None, scale=None):
        return self.forward(attn, hidden_states, pose_feature, 
                            encoder_hidden_states, attention_mask, temb, scale)

    def forward(self, attn, hidden_states, pose_feature,
                    encoder_hidden_states, attention_mask, temb, scale):
        ...
        B, ehs_seq_len, _ = encoder_hidden_states.shape
        attention_mask = attn.prepare_attention_mask(attention_mask, ehs_seq_len, B)
...
class PoseAdaptorAttnProcessor2_0(nn.Module):
    ...
    def forward(
        ...
        query = query.view(B, -1, attn.heads, head_dim).transpose(1, 2)        # [bs, seq_len, nhead, head_dim] -> [bs, nhead, seq_len, head_dim]
        key   =   key.view(B, -1, attn.heads, head_dim).transpose(1, 2)
        value = value.view(B, -1, attn.heads, head_dim).transpose(1, 2)
        ...
        hidden_states = hidden_states.transpose(1, 2).reshape(B, -1, attn.heads * head_dim)    # [bs, seq_len, dim]
        hidden_states = hidden_states.to(query.dtype)
...
if __name__ == "__main__":
    import inspect
    sig = inspect.signature(PoseAdaptorAttnProcessor.forward)
    print(sig)
