...
class TransformerSpatioTemporalModelPoseCond(nn.Module):
    ...
    def forward(
        ...
        time_context_first_timestep = time_context[None, :].reshape(
            batch_size, num_frames, -1, time_context.shape[-1])[:, 0]   # [bs, frame, c]
    
        time_context = time_context_first_timestep[:, None].broadcast_to(
            batch_size, 
            height * width, 
            time_context.shape[-2], time_context.shape[-1])             # [bs, h * w, 1, c]
        ...
        for block, temporal_block in zip(self.transformer_blocks, self.temporal_transformer_blocks):
            if self.training and self.gradient_checkpointing:
                hidden_states = checkpoint(block, hidden_states, None,
                                          encoder_hidden_states, None, use_reentrant=False)
            else:
                hidden_states = block(
