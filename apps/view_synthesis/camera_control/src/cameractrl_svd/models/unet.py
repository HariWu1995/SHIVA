...
from .unet_3d_blocks import get_down_block, get_up_block, UNetMidBlockSpatioTemporalPoseCond
...
#     from .attention_processor import PoseAdaptorAttnProcessor2_0 as PoseAdaptorAttnProcessor
#     from .attention_processor import AttnProcessor2_0 as CustomizedAttnProcessor
# else:
...
class UNetSpatioTemporalConditionModelPoseCond(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def attn_processors(self) -> Dict[str, AttentionProcessor]:
        r"""
        Returns:
            `dict` of attention processors: A dictionary containing all attention processors 
                                            used in the model with indexed by its weight name.
        """
        # set recursively
        processors = {}
        ...
        if add_spatial:
            for processor_key in self.attn_processors.keys():
                ...
                if add_pose_adaptor and enable_xformers:
                    all_attn_processors[processor_key] = PoseAdaptorXFormersAttnProcessor(
                    ...
                    all_attn_processors[processor_key] = PoseAdaptorAttnProcessor(
                        hidden_size=hidden_size,
                        pose_feature_dim=pose_feature_dim,
                        cross_attention_dim=cross_attention_dim,
                        **attention_processor_kwargs
