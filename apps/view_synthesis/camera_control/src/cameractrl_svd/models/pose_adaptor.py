...
class PoseAdaptor(nn.Module):
    ...
    def forward(self, noisy_latents, c_noise, encoder_hidden_states, added_time_ids, pose_embedding):
        ...
        noise_pred = self.unet(noisy_latents, c_noise,
...
class Downsample(nn.Module):
    ...
    """
    def __init__(self, in_channels, use_conv, dims=2, out_channels=None, padding=1):
        super().__init__()
        ...
        if use_conv:
            self.op = conv_nd(dims, self.in_channels, self.out_channels, 3, stride=stride, padding=padding)
        else:
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, down, kernel_size=3, sk=False, use_conv=True):
        super().__init__()

        ps = kernel_size // 2
        if in_channels != out_channels or sk == False:
...
class CameraPoseEncoder(nn.Module):
        ...
        for i in range(len(channels)):
            ...
            for j in range(nums_rb):
                ...
                if j == 0 and i != 0:
                    ...
                    conv_layer = ResnetBlock(in_dim, out_dim, down=True, kernel_size=ksize, sk=sk, use_conv=use_conv)
    
                elif j == 0:
                    in_dim = channels[0]
                    out_dim = int(channels[i] / compression_factor)
                    conv_layer = ResnetBlock(in_dim, out_dim, down=False, kernel_size=ksize, sk=sk, use_conv=use_conv)
    ...
    def forward(self, x):
        ...
        features = []
        for res_block, attention_block in zip(self.encoder_down_conv_blocks, 
                                              self.encoder_down_attention_blocks):
