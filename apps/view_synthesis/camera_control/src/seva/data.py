...
from .utils import _get_rel_paths
from .geometry import transform_cameras, similarity_from_cameras, \
                       transform_points, align_principle_axes
...
class BaseParser(object):
    ...
    def __init__(
        ...
        self.imsize_dict: Dict[int, Tuple[int, int]] = {}   # Dict of camera_id -> (width, height)
        ...
        self.roi_undist_dict: \
            Dict[int, Tuple[int, int, int, int]] = dict()   # Dict of camera_id -> (x, y, w, h)
...
class DirectParser(BaseParser):
    ...
    def __init__(
        ...
        for i, img in enumerate(imgs):
            self.image_names.append(f"{i:06d}")
            self.image_paths.append(f"null")
...
class COLMAPParser(BaseParser):
    ...
    def __init__(
        ...
        for k in imdata:
            ...
            assert (camtype == "perspective"  # type: ignore
        ...
        colmap_dir = os.path.join(data_dir, image_folder)
        image_dir = os.path.join(data_dir, image_folder + image_dir_suffix)
        for d in [image_dir, colmap_dir]:
        ...
        # Normalize the world space.
        if normalize:
            T1 = tr_1 = similarity_from_cameras(camtoworlds)
            camtoworlds = transform_cameras(T1, camtoworlds)
            points = transform_points(T1, points)

            T2 = align_principle_axes(points)
            camtoworlds = transform_cameras(T2, camtoworlds)
            points = pts = transform_points(T2, points)
            transform = T2 @ T1
        else:
            transform = np.eye(4)
...
class ReconfusionParser(BaseParser):
    ...
    def __init__(self, data_dir: str, normalize: bool = False):
        ...
        def get_num(p):
            p_num = p.split("_")[-1].removesuffix(".json")
            return int(p_num) if p_num.isdigit() else p_num

        num_input_frames = [
            get_num(p) for p in sorted(glob(osp.join(data_dir, "train_test_split_*.json")))
        ]

        self.splits_per_num_input_frames = {}
        for n_frames in num_input_frames:
            with open(osp.join(data_dir, f"train_test_split_{n_frames}.json")) as f:
                self.splits_per_num_input_frames[n_frames] = json.load(f)
        ...
        for frame in metadata["frames"]:
            ...
            if "applied_transform" in metadata:
                applied_transform = np.concatenate([metadata["applied_transform"], [[0, 0, 0, 1]]], axis=0)
        ...
        path_bounds = osp.join(data_dir, "bounds.npy")
        if osp.exists(path_bounds):
            self.bounds = np.load(path_bounds)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if isinstance(parser, ReconfusionParser):
            ...
            self.indices = ids_per_split["train_ids" if split == "train" else "test_ids"]
        else:
            indices = np.arange(len(self.parser.image_names))
            if split == "train":
                self.indices = (
                    indices[indices % self.parser.test_every != 0]
                    if self.parser.test_every is not None else indices
                )
            else:
                self.indices = (
                    indices[indices % self.parser.test_every == 0]
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        if params is not None and len(params) > 0:
            ...
            x, y, \
            w, h = self.parser.roi_undist_dict[camera_id]
        ...
        if self.patch_size is not None:
            ...
            image = image[y : y + self.patch_size, 
                          x : x + self.patch_size]
            K[0, 2] -= x
            K[1, 2] -= y
        ...
        if self.load_mono_disps:
            data["mono_disps"] = torch.from_numpy(
                                self.parser.mono_disps[index]).float()  # type: ignore
        return data
