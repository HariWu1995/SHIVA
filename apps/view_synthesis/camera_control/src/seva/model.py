...
from .modules.layers import (
    Upsample, Downsample, ResBlock, GroupNorm32,
    TimestepEmbedSequential, timestep_embedding,
)
...
def load_model(
    ...
    if verbose:
        warning(missing, unexpected)
    return model


def warning(missing: list[str], unexpected: list[str]) -> None:
...
class SGMWrapper(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: dict, **kwargs) -> torch.Tensor:
        ...
        return self.module(x, t=t, y=c["crossattn"], dense_y=c["dense_vector"], **kwargs)
