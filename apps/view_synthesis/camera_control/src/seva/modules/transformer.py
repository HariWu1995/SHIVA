...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        d_ctx = context_dim
        self.attn1 = Attention(query_dim=dim, context_dim=None, heads=n_heads, dim_head=d_head, dropout=dropout)
        self.attn2 = Attention(query_dim=dim, context_dim=d_ctx, heads=n_heads, dim_head=d_head, dropout=dropout)
...
class TransformerBlockTimeMix(nn.Module):
    ...
    def __init__(
        ...
        self.attn1 = Attention(query_dim=inner_dim, context_dim=None, heads=n_heads, dim_head=d_head, dropout=dropout)
        self.attn2 = Attention(query_dim=inner_dim, context_dim=ctx_dim, heads=n_heads, dim_head=d_head, dropout=dropout)
...
class MultiviewTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_blocks = nn.ModuleList([
            TransformerBlock(inner_dim, n_heads, d_head, context_dim=context_dim, dropout=dropout)
        ...
        self.time_mix_blocks = nn.ModuleList([
            TransformerBlockTimeMix(inner_dim, n_heads, d_head, context_dim=context_dim, dropout=dropout)
            for _ in range(depth)
        ])
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, num_frames: int) -> torch.Tensor:
        ...
        time_context_1step = time_context[::num_frames]
        time_context = repeat(time_context_1step, "b ... -> (b n) ...", n=h*w)
