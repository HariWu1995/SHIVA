...
from .main import run_one_scene
from .eval import (
    IS_TORCH_NIGHTLY,
...
def main(
    ...
    **overwrite_options,
    ...
    if data_items is not None:
        ...
        scenes = glob(osp.join(data_path, "*"))
    ...
    for scene in tqdm(scenes):
        ...
        img_paths = sorted(glob(osp.join(save_path_scene, "samples-rgb", "*.png")))
        if len(img_paths) != len(c2ws):
            input_img_paths = sorted(
                glob(osp.join(save_path_scene, "input", "*.png"))
            )
