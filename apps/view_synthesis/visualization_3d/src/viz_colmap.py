...
Visualize COLMAP sparse reconstruction outputs. 
    To get demo data, see `./assets/download_colmap_garden.sh`.
"""
import random
import time
...
def main(
    colmap_path: str | Path = Path(__file__).parent / "assets/colmap_garden/sparse",
    images_path: str | Path = Path(__file__).parent / "assets/colmap_garden/images",
    ...
    gui_points = server.gui.add_slider("Max points", min=1, max=len(points3d), step=1, initial_value=min(len(points3d), 50_000))
    gui_frames = server.gui.add_slider("Max frames", min=1, max=len(images), step=1, initial_value=min(len(images), 100))
    gui_point_size = server.gui.add_slider("Point size", min=0.01, max=0.1, step=0.001, initial_value=0.05)
    ...
    def visualize_frames() -> None:
        ...
        for img_id in tqdm(img_ids):
            ...
            T_world_camera = vtf.SE3.from_rotation_and_translation(vtf.SO3(img.qvec), img.tvec).inverse()
