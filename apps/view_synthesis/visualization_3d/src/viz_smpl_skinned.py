...
SMPL visualizer (Skinned Mesh)
    https://github.com/nerfstudio-project/viser/blob/main/examples/25_smpl_visualizer_skinned.py
...
class SmplHelper:
    ...
    Does not include blend skinning.
    """
    def __init__(self, model_path: Path) -> None:
        assert model_path.suffix.lower() == ".npz", f"Model should be an .npz file! Not {model_path}"
...
def make_gui_elements(
    ...
    with tab_group.add_tab("View", viser.Icon.VIEWFINDER):
        ...
        gui_control_size = server.gui.add_slider("Handle size", min=0.0, max=10.0, step=0.01, initial_value=1.0)
        ...
        def _(_):
            for control in transform_controls:
                ...
                control.scale = 0.2 * (0.75 ** prefixed_joint_name.count("/")) * gui_control_size.value
    ...
    with tab_group.add_tab("Shape", viser.Icon.BOX):
        ...
        for i in range(num_betas):
            beta = server.gui.add_slider(f"beta{i}", min=-5.0, max=5.0, step=0.01, initial_value=0.0)
    ...
    with tab_group.add_tab("Joints", viser.Icon.ANGLE):
        ...
        for i in range(num_joints):
            ...
            def set_callback_in_closure(i: int) -> None:
                @gui_joint.on_update
                def _(_):
                    transform_controls[i].wxyz = tf.SO3.exp(np.array(gui_joints[i].value)).wxyz
                    out.changed = True
    ...
    for i in range(num_joints):
        ...
        if i > 0:
            prefixed_joint_name = prefixed_joint_names[parent_idx[i]] + "/" + prefixed_joint_name
    ...
    out = GuiElements(gui_rgb, gui_wireframe, gui_betas, gui_joints,
                      transform_controls=transform_controls, changed=True, betas_changed=False)
    return out
...
def main(
    ...
    while True:
        ...
        if gui_elements.betas_changed:
            v_tpose, j_tpose = model.get_tpose(np.array([gui_beta.value for gui_beta in gui_elements.gui_betas]))
        ...
        smpl_outputs = model.get_outputs(
            ...
            joint_rotmats=np.stack([tf.SO3.exp(np.array(x.value)).as_matrix() 
                                                    for x in gui_elements.gui_joints], axis=0),
        )
        ...
        for i, control in enumerate(gui_elements.transform_controls):
            ...
            mesh_handle.bones[i].wxyz = tf.SO3.from_matrix(smpl_outputs.T_world_joint[i, :3, :3]).wxyz
