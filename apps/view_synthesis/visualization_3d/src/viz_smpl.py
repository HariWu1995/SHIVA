...
SMPL model visualizer
    https://github.com/nerfstudio-project/viser/blob/main/examples/08_smpl_visualizer.py
...
def make_gui_elements(
    ...
    out = GuiElements(gui_rgb, gui_wireframe, gui_betas, gui_joints,
                      transform_controls=transform_controls, changed=True)
    return out
...
def main(
    ...
    try:
        ...
        print("\n\n[WARNING] Skipp `vertex_selector` due to", str(e))
    ...
    while True:
        ...
        smpl_outputs = model.get_outputs(
            ...
            joint_rotmats=tf.SO3.exp(np.array([x.value for x in gui_elements.gui_joints])).as_matrix(),
        )
