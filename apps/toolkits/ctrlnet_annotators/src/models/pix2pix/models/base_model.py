...
class BaseModel(ABC):
    ...
    def __init__(self, opt, **kwargs):
        ...
        self.save_path = kwargs.get("model_path", None)
        self.save_dir = os.path.join(opt.checkpoints_dir, opt.name)  # save all the checkpoints to save_dir
        self.device = kwargs.get("device", 
                            torch.device(f"cuda:{self.gpu_ids[0]}" if self.gpu_ids else "cpu"))
        
        # NOTE: with [scale_width], input images might have different sizes, 
