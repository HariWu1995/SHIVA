...
def draw_animal_poses(animals: List[List[Keypoint]], H: int, W: int) -> np.ndarray:
    canvas = np.zeros(shape=(H, W, 3), dtype=np.uint8)
    for animal_pose in animals:
        canvas = draw_animal_pose(canvas, animal_pose)
    return canvas


def draw_animal_pose(canvas: np.ndarray, keypoints: List[Keypoint]) -> np.ndarray:
    ...
    for ind, (i, j) in enumerate(keypointPairsList):
        ...
        if p1 is not None and p2 is not None:
            cv2.line(canvas, (int(p1.x), int(p1.y)),
                             (int(p2.x), int(p2.y)), colorsList[ind], 5)
    return canvas
...
class AnimalPose:
    ...
    def __call__(self, image) -> List[AnimalPoseResult]:
        # https://github.com/ultralytics/ultralytics/blob/main/ultralytics/cfg/datasets/coco.yaml
        # 14-23: bird - cat - dog - horse - sheep - cow - elephant - bear - zebra - giraffe
        detect_classes = list(range(14, 23 + 1))
        det_result = inference_detector(self.session_det, image, detect_classes=detect_classes)

        if (det_result is None) \
        or (det_result.shape[0] == 0):
            return []

        keypoint_sets, scores = inference_pose(self.session_pose, det_result, image, self.model_input_size)
