...
class Body(object):
    ...
    def __call__(self, image):
        ...
        for m in range(len(multiplier)):
            scale = multiplier[m]
            imageToTest = util.smart_resize_k(image, fx=scale, fy=scale)
            ...
            heatmap = util.smart_resize(heatmap, (image.shape[0], image.shape[1]))

            # paf = np.transpose(np.squeeze(net.blobs[output_blobs.keys()[0]].data), (1, 2, 0))  # output 0 is PAFs
            paf = np.transpose(np.squeeze(Mconv7_stage6_L1), (1, 2, 0))  # output 0 is PAFs
            paf = util.smart_resize_k(paf, fx=stride, fy=stride)
            paf = paf[:imageToTest_padded.shape[0] - pad[2], :imageToTest_padded.shape[1] - pad[3], :]
            paf = util.smart_resize(paf, (image.shape[0], image.shape[1]))
            ...
            if (nA != 0 and nB != 0):
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        score_with_dist_prior = sum(score_midpts) / len(score_midpts) + min(0.5 * image.shape[0] / norm - 1, 0)
