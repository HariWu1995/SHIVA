...
class StableDiffusionBrushNetPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if mask is None:
            mask = kwargs.get('mask_image', None)
        ...
        conditioning_latents = conditioning_latents.to(device=device)
        mask = F.interpolate(original_mask, size=(conditioning_latents.shape[-2], 
                                                  conditioning_latents.shape[-1]))
        conditioning_latents = torch.concat([conditioning_latents, mask], dim=1)
