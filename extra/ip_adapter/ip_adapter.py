...
from transformers import CLIPImageProcessor, CLIPVisionModelWithProjection as CLIPVisionModel
...
class IPAdapter:
    ...
    def __init__(self, sd_pipe, image_encoder_path, ip_ckpt, device, num_tokens=4, target_blocks=["block"]):
        ...
        # load image encoder
        self.image_encoder = CLIPVisionModel.from_pretrained(self.image_encoder_path).to(self.device, dtype=torch.float16)
    ...
    def generate(
        ...
        **kwargs,
        ...
        image_prompt_embeds, \
        uncond_image_prompt_embeds = self.get_image_embeds(
                                                    pil_image = pil_image, 
...
class IPAdapter_CS:
    ...
    def __init__(
        ...
        if content_image_encoder_path is not None:
            self.content_image_encoder = AutoModel.from_pretrained(content_image_encoder_path).to(self.device, dtype=torch.float16)
            self.content_image_processor = AutoImageProcessor.from_pretrained(content_image_encoder_path)
        else:
            self.content_image_encoder = CLIPVisionModel.from_pretrained(self.image_encoder_path).to(self.device, dtype=torch.float16)
        ...
        # image proj model
        self.content_image_proj_model = self.init_proj(self.num_content_tokens, content_or_style_='content', model_resampler=self.content_model_resampler)
        self.style_image_proj_model = self.init_proj(self.num_style_tokens, content_or_style_='style', model_resampler=self.style_model_resampler)
            ...
            if self.controlnet_adapter is False:
                ...
                for name in self.pipe.controlnet.attn_processors.keys():
                    cross_attention_dim = None if name.endswith("attn1.processor") else \
                                            self.pipe.controlnet.config.cross_attention_dim
                    if name.startswith("mid_block"):
    ...
    def get_image_embeds(self, pil_image=None, clip_image_embeds=None, 
                                            content_prompt_embeds=None, content_or_style_=''):
        # if pil_image is not None:
        #     if isinstance(pil_image, Image.Image):
        #         pil_image = [pil_image]
        #     clip_image = self.clip_image_processor(images=pil_image, return_tensors="pt").pixel_values
        ...
        if content_or_style_ == 'content':
            if pil_image is not None:
                ...
                if self.content_image_proj_model is not None:
                    ...
                    clip_image_embeds = self.content_image_encoder(clip_image.to(self.device, dtype=torch.float16)).image_embeds
                else:
                    clip_image = self.clip_image_processor(images=pil_image, return_tensors="pt").pixel_values
                    clip_image_embeds = self.image_encoder(clip_image.to(self.device, dtype=torch.float16)).image_embeds
            else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        uncond_content_image_prompt_embeds = uncond_content_image_prompt_embeds.view(bs_embed * num_samples, seq_len, -1)
        ...
        uncond_style_image_prompt_embeds = uncond_style_image_prompt_embeds.view(bs_embed * num_samples, seq_style_len, -1)
        ...
        with torch.inference_mode():
            ...
            prompt_embeds = [prompt_embeds_, content_image_prompt_embeds, style_image_prompt_embeds]
            prompt_embeds = torch.cat(prompt_embeds, dim=1)

            negative_prompt_embeds = [negative_prompt_embeds_, uncond_content_image_prompt_embeds, uncond_style_image_prompt_embeds]
...
class IPAdapterXL_CS(IPAdapter_CS):
    ...
    def generate(
        ...
        **kwargs,
        ...
        uncond_content_image_prompt_embeds = self.get_image_embeds(pil_content_image,
                                                                        content_image_embeds,
                                                                        content_or_style_='content')

        style_image_prompt_embeds, \
        uncond_style_image_prompt_embeds = self.get_image_embeds(pil_style_image,
                                                                    style_image_embeds,
        ...
        with torch.inference_mode():
            ...
            prompt_embeds = [prompt_embeds, content_image_prompt_embeds, style_image_prompt_embeds]
            prompt_embeds = torch.cat(prompt_embeds, dim=1)

            negative_prompt_embeds = [negative_prompt_embeds, uncond_content_image_prompt_embeds, uncond_style_image_prompt_embeds]
            negative_prompt_embeds = torch.cat(negative_prompt_embeds, dim=1)
...
class CSComposer(IPAdapterXL_CS):
    ...
    def __init__(
        ...
        super().__init__(sd_pipe, image_encoder_path, ip_ckpt, device,
                        ...
                        content_image_encoder_path, controlnet_adapter,
                        controlnet_target_content_blocks, controlnet_target_style_blocks,
                        content_model_resampler, style_model_resampler)
